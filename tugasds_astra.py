...
# ==========================================
# 3. SIDEBAR & SISTEM FILTER SLIDER RENTANG
# ==========================================
st.sidebar.title("Filter Analisis")

daftar_kuartal = df_plot.index.tolist()

# Menggunakan select_slider agar pengguna bisa menggeser rentang kuartal
rentang_terpilih = st.sidebar.select_slider(
    "Pilih Rentang Kuartal:",
    options=daftar_kuartal,
    value=(daftar_kuartal[0], daftar_kuartal[-1]) # Posisi awal: dari ujung kiri ke ujung kanan
)
...
# Filter Dataframe berdasarkan rentang yang digeser
...
# Kalkulasi Nilai untuk Rentang Terpilih
...
# Logika Pembanding: Jika user hanya pilih 1 kuartal tunggal, kita hitung Growth (QoQ)
...
if is_single_quarter and idx_awal > 0:
    ...
    # Ambil data kuartal sebelumnya untuk perbandingan
    ...
    # Jika user menarik rentang slider (>1 kuartal), jadikan total agregat tanpa pertumbuhan
...
with col1:
    tampilkan_kpi("Total Pendapatan", pend_ini, pend_lalu, status_total=status_total)
with col2:
    tampilkan_kpi("Laba Kotor", laba_kotor_ini, laba_kotor_lalu, status_total=status_total)
with col3:
    tampilkan_kpi("Laba Bersih", laba_bersih_ini, laba_bersih_lalu, status_total=status_total)
with col4:
    tampilkan_kpi("Net Profit Margin", npm_ini, npm_lalu, format_persen=True, status_total=status_total)
